# Wiki Index

> Content catalog. Every wiki page listed under its type with a one-line summary.
> Read this first to find relevant pages for any query.
> Last updated: 2026-04-28 | Total pages: 26

## Concepts (8)
- [[vibe-coding-beginner-methodology]] — 바이브 코딩 초보자 방법론: 5단계 워크플로우·3계층 프롬프트·보안·한계 종합 가이드 (2026-04-28, NotebookLM 6소스 분석)
- [[hyperagents-design-study]] — HyperAgents 메타 에이전트 설계 분석: 계층적 워크플로우·동적 도구 할당·체크포인트 시스템
- [[agent-memory-systems]] — 에이전트 메모리 설계 패턴: 단기·장기·작업 메모리의 계층적 아키텍처와 캐시 전략
- [[ai-coding-collaboration-patterns]] — AI-인간 협업 코딩 패턴: Pair Programming, Agent Delegation, Review Loops
- [[prompt-engineering-systematic]] — 체계적 프롬프트 엔지니어링: Chain-of-Thought, Few-Shot, Constraint Specification
- [[beginners-systematic-ai-methodology]] — 초보자 체계적 AI 협업 방법론 (연구 초안)

## Entities (13)
- [[boop-agent]] — Boop Agent: iMessage 기반 오픈소스 개인 비서. Claude Agent SDK + Dispatcher-Worker 아키텍처 (2026-04-28)
- [[research-agent-설계-계획]] — 바이브코딩+Senior-level AI 협업 연구 설계 (2026-04-27, 2-Stage Review 완료)
- [[hyperagents-meta-agent-analysis]] — HyperAgents 프레임워크 심층 분석 (2026-04-27)
- [[smart-video-speed-bug]] — YouTube DVR 라이브 감지 버그 (LiveSync.ts, 2026-04-26)
- [[claude-code-sessions]] — Claude Code 세션 관리·훅·설정 (2026-04-26)
- [[llm-brain-architecture]] — HermesBrain LLM 위키 아키텍처 (2026-04-24)
- [[obscura-web-scraper]] — Obscura 브라우저 자동화 스크래퍼 (2026-04-24)
- [[nlwflow-pipeline]] — NotebookLM → LLM Wiki 파이프라인 도구 (2026-04-28)
- [[paperclip-discord-plugin]] — Paperclip Discord 플러그인 (버그: MESSAGE_CONTENT_INTENT)
- [[mlx-local-llm-setup]] — M1 Mac 로컬 LLM 추론 환경 (oMLX)
- [[smart-video-speed-project]] — Smart Video Speed 확장 프로그램 프로젝트 (2026-04-26)
- [[karpathy-guidelines-skill]] — Karpathy Guidelines: Think→Simple→Surgical→Goal-Driven (2026-04-26)
- [[subagent-driven-dev-pattern]] — Subagent-Driven Development 패턴 (2026-04-26)

## Comparisons (2)
- [[hermes-vs-boop]] — Hermes Agent vs Boop Agent: 아키텍처·Memory·통합·안전장치 전방위 비교 (2026-04-28)
- [[note-wiki-vibe-coding-beginner-methodology]] — 바이브 코딩 6개 소스 비교분석 (대상·강점·도구 추천·연구 간극)

## Queries (3)
- [[vibe-coding-beginner-checklist]] — 바이브 코딩 초보자 실행 체크리스트 (시작→환경→프롬프트→검증→보안→배포→개선)
- [[note-wiki-vibe-coding-beginner-methodology-checklist]] — (nlwflow 자동 생성 체크리스트)

## Raw Sources
- `raw/papers/ai-novice-to-senior-research` — 초보자 AI 협업 논문 12편
- `raw/notebooklm-vibe-coding-report` — NotebookLM 바이브 코딩 리포트 (2026-04-28)
- `/Users/bricoleur/artifacts/7ebb8346-3fa1-4f7a-9809-baa38faca430/` — NotebookLM 원본 아티팩트