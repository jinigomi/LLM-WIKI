...
- [[matt-pocock-diagnose]] — 재현→최소화→가설→측정→수정→회귀 테스트의 디버깅 루프
...
- [[matt-pocock-to-prd]] — 모듈 구조 고려한 PRD 생성
- [[matt-pocock-zoom-out]] — 전체 시스템 맥락에서 코드를 설명하도록 지시
...
| 용어 | 정의 | 회피할 단어 |
|------|------|------------|
| **Issue tracker** | 이슈를 호스팅하는 도구 (GitHub Issues, Linear 등) | backlog manager, backlog backend |
| **Issue** | 단일 작업 단위 | ticket |
| **Triage role** | 이슈에 적용되는 상태 머신 라벨 | — |
| **Module** | 인터페이스 + 구현을 가진 모든 것 | component, service, API |
| **Seam** | 인터페이스가 존재하는 위치. 동작을 in-place 편집 없이 변경 가능 | boundary |
